...
Jps::Jps(int16_t len, int16_t wid) {
  len_ = len;
  wid_ = wid;
  obstacle_ = vector<vector<bool>>(len, vector<bool>(wid, false));
}
...
void Jps::fneig(Pos p, Pos d, function<void(Pos, Pos)> cb) {
  ...
  p11.x_ = p1.x_ + d.x_;
  p11.y_ = p1.y_ + d.y_;
  if (inarea(p1) && obstacle(p1) && !obstacle(p11)) {
    Pos nd;
    nd.x_ = d.x_ + d.y_;
    nd.y_ = d.y_ + d.x_;
    cb(p11, nd);
  ...
  p21.x_ = p2.x_ + d.x_;
  p21.y_ = p2.y_ + d.y_;
  if (inarea(p2) && obstacle(p2) && !obstacle(p21)) {
    Pos nd;
    nd.x_ = d.x_ - d.y_;
    nd.y_ = d.y_ - d.x_;
    cb(p21, nd);
  ...
  while (true) {
    if (d.x_ == 0 || d.y_ == 0) {
      ...
      if (r1 || r2) {
        ...
        s.x_ += d.x_;
        s.y_ += d.y_;
        if (obstacle({s.x_, s.y_})) return true;
        s.cost_ += HWEI;
        addjp(s, pstart);
        return true;
      }
...
void Jps::find() {
  reset();
  if (obstacle(start_) || obstacle(end_)) return;
