...
struct World {
  ...
  void init_world(int16_t len, int16_t wid) {
    len_ = len;
    wid_ = wid;
    map_ = vector<vector<bool>>(wid, vector<bool>(len));
  }
  ...
  void set_block(int16_t minx, int16_t miny, int16_t maxx, int16_t maxy,
                 bool v) {
...
struct Jps {
  World world_;
  vector<vector<vector<int16_t>>> jp_cache_;
  ...
  void add_jp_cache(Pos p, Pos dir);
  vector<int16_t>* get_jp_cache(Pos p, Pos dir);
  void init_jp_cache();
